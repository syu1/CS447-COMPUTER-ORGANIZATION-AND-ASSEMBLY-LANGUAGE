...
# Start the loop. Use a for loop using $s1 as a sentry
	loop1: beq $s1, $t0, done1
	#############################################################################
	##########GOING TO TEST SHIFT LEFT THEN RIGHT TO ISOLATE THE BIT#############
	#############################################################################
	#one register for total, #one register for shifting the operand $s0
	and $t3,$s1, 1
	beq $t3, 1, do_operation
	beq $t3, 0, jump_to_shift
	do_operation:
	addi $t8,$s0,0
	j jump_to_shift
...
syscall
 jr $ra
#returns $t8 as the correct number i bleicec
